...
#testtable <- readHTMLTable(doc = "html_shots/season_2000_01_games.htm")
...
for (i in games) {
   ...
   for (ii in 1:34) {
      ...
      if (ii == 1) {
         spiele <- cbind(teams, ii, saison, "Heim", tore)
      } else {
         spiele <- rbind(spiele, cbind(teams, ii, saison, "Heim", tore))
      }
...
# Variable benennen
names(alle_spiele) <- c("team_a", "team_b", "spieltag", "saison", "heim_gast", "tore_a", "tore_b", "sun_a", "sun_b")


## hier nochmal alle Spiele drehen, um aus der Heimmannschaft die Gastmannschaft etc zu machen
temp <- alle_spiele[,c(2,1,3,4,5,7,6,9,8)]
names(temp) <- c("team_a", "team_b", "spieltag", "saison", "heim_gast", "tore_a", "tore_b", "sun_a", "sun_b")
