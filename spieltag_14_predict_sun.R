...
spieltag <- data.frame(readHTMLTable(doc = "http://www.bulibox.de/statistik/bundesliga-ergebnisse.php?liga=1&spieltag=14"))[, 2:3]
table <- data.frame(readHTMLTable(doc = "http://www.bulibox.de/tabelle/dynamische-bundesligatabelle.php?ab=1&bis=13&liga=1&Abfrage=OK"), stringsAsFactors=FALSE)[, 1:6]
...
loses <- as.numeric(unlist(lapply(sun, function(x) x[3])))
...
predictdata$wins_pro_a <- predictdata$wins_a / 13
predictdata$points_pro_a <- predictdata$points_a / (13 * 3)

predictdata$wins_pro_b <- predictdata$wins_b / 13
predictdata$points_pro_b <- predictdata$points_b / (13 * 3)
